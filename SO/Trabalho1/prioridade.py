import sys
from ProcessesP import Processes


#@file: prioridade.py
#@author: Arthur Petroli
#@date: 29 May 24
#@brief: Priority Scheduler implementation in Python for the Operating Systems discipline at UTFPR-AP.
...
def leituraArq(arq):
    ...
    prioridades = linhas[3].strip().split(';')

    for i in range(min(len(nomes), len(temposCPU), len(prioridades)) - 1):
        nome = nomes[i + 1]
        ...
        try:
            ...
            if prioridade < 0:
                print(f"Erro na prioridade do processo {nome}\n")
                continue
        except ValueError:
            print(f"Erro na prioridade do processo {nome}\n")
            continue

        processo = Processes(nome, tempoCPU, tempoCheg, prioridade)
        p.append(processo)

    return p

def priorityScheduler(processos):
    tempo_atual = 0
    fila_execucao = []
    relatorio = []
    ...
    while processos or fila_execucao:
        ...
        if fila_execucao:
            fila_execucao.sort(key=lambda x: x.prioridade)     
...
def escritaRelatorio1(arq, processos, relatorio):
    with open(arq, 'w') as saida:
        saida.write("Processos na Fila do Priority Scheduler:\n")
        ...
        for processo in processos:
            saida.write(f"{processo.prioridade} ")
        saida.write("\n\nLinha do tempo:\n")
...
if __name__ == "__main__":
    ...
    relatorio= priorityScheduler(processos.copy())
